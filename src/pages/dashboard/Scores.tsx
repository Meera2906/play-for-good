...
import { z } from 'zod';
import { Target, Calendar, MapPin, Plus, Trash2, Trophy, Loader2, AlertCircle, Lock } from 'lucide-react';
          ...
          <div className="glass-card overflow-hidden">
            ...
            {scores.length === 0 ? (
              <div className="p-12 text-center text-on-surface-variant">
                ...
                <p>No scores logged yet. Your latest 5 scores will appear here.</p>
              </div>
            ) : (
                ...
                {scores.map((score, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-6">
                      <div className="w-14 h-14 bg-surface-container-highest rounded-2xl flex items-center justify-center font-display font-bold text-xl text-primary shadow-lg border border-white/5">
                        {score.stableford_points}
                      </div>
                      <div>
                        <h4 className="font-bold text-lg">{score.course_name}</h4>
                        <p className="text-sm text-on-surface-variant font-sans">{formatDate(score.date)}</p>
                      </div>
                    </div>
                    <button
                      onClick={() => handleDelete(score.id)}
                      className="w-10 h-10 rounded-xl bg-white/5 text-on-surface-variant hover:bg-red-500/20 hover:text-red-500 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
                      title="Delete Score"
                    >
            ...
            {scores.length === 5 && (
              <div className="p-6 bg-secondary/10 border-t border-secondary/20 flex items-center justify-center gap-3">
                <Trophy className="w-5 h-5 text-secondary" />
                <span className="text-sm text-secondary font-medium">You have reached the maximum 5 active scores. New entries will replace the oldest round.</span>
              </div>
            )}
          </div>
        </div>
